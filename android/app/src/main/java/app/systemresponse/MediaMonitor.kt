...
data class CallState(val known: Boolean, val busy: Boolean)
object CallGuard {
    fun read(context: Context): CallState {
        val audio = context.getSystemService(AudioManager::class.java)
        val voice = audio.mode != AudioManager.MODE_NORMAL
        if (!context.packageManager.hasSystemFeature(PackageManager.FEATURE_TELEPHONY)) return CallState(true, voice)
        if (context.checkSelfPermission(Manifest.permission.READ_PHONE_STATE) != PackageManager.PERMISSION_GRANTED) return CallState(false, voice)
        return try {
            val state = context.getSystemService(TelephonyManager::class.java).callState
            CallState(true, voice || state != TelephonyManager.CALL_STATE_IDLE)
        } catch (_: Exception) { CallState(false, voice) }
    }
}
