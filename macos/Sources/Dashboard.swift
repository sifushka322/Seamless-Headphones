...
struct Dashboard: View {
        ...
        HStack(spacing: 0) {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 24) {
                    ...
                    HStack { Text("SEAMLESS HEADPHONES").tracking(1.7); Spacer(); Text(model.demo ? "ДЕМО · БЕЗ КОМАНД УСТРОЙСТВАМ" : "0.2.0 · ЛОКАЛЬНО ПО BLUETOOTH") }
    ...
    private var sidebar: some View {
        VStack(alignment: .leading, spacing: 28) {
            HStack(spacing: 10) {
                Image(systemName: "waveform").font(.system(size: 21, weight: .semibold)).foregroundStyle(accent).frame(width: 42, height: 42).background(accent.opacity(0.12), in: RoundedRectangle(cornerRadius: 13))
        ...
        VStack(spacing: 20) {
            ...
            HStack(spacing: 24) {
                ...
                ZStack {
                    ...
                    Image(systemName: "headphones").font(.system(size: 70, weight: .ultraLight)).foregroundStyle(accent)
            ...
            HStack(alignment: .top, spacing: 14) {
                ...
                Spacer()
                if model.autoPaused { Button("Возобновить", action: model.resumeAuto) }
    ...
    private var automation: some View {
        VStack(alignment: .leading, spacing: 20) {
            VStack(alignment: .leading, spacing: 18) {
                ...
                if model.autoPaused { Button("Возобновить после проверки", action: model.resumeAuto) }
            ...
            VStack(alignment: .leading, spacing: 18) {
                ...
                Text("Ручная команда имеет приоритет на 2 минуты. Во время разговора или активного микрофона автоматизация приостановлена.").font(.caption).foregroundStyle(.secondary)
            }.padding(22).card(surface, line)
            VStack(alignment: .leading, spacing: 16) {
                ...
                LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], alignment: .leading, spacing: 15) {
                    ForEach(MediaMonitor.sources, id: \.0) { id, name in
                ...
                Slider(value: $model.cooldown, in: 15...60, step: 5).accessibilityLabel("Пауза между автоматическими передачами")
            }.padding(22).card(surface, line)
        }
    }
    private var devices: some View {
        VStack(alignment: .leading, spacing: 20) {
            VStack(alignment: .leading, spacing: 18) {
                ...
                Picker("Наушники", selection: $model.selected) { Text("Выбрать устройство").tag(""); ForEach(model.headsets) { Text($0.name).tag($0.id) } }.disabled(model.busy)
            ...
            VStack(alignment: .leading, spacing: 18) {
                ...
                HStack {
                    if model.enabled { Button("Показать ключ") { model.showPairing.toggle() }; Button("Остановить связь", action: model.disable) }
    ...
    private var settings: some View {
        VStack(alignment: .leading, spacing: 20) {
            ...
            VStack(alignment: .leading, spacing: 14) {
                HStack { Text("История и диагностика").font(.headline); Spacer(); Button("Скопировать", action: model.copyDiagnostics); Button("Очистить") { model.events.removeAll() } }
...
private extension View {
    func card(_ fill: Color, _ border: Color) -> some View { background(fill, in: RoundedRectangle(cornerRadius: 18)).overlay(RoundedRectangle(cornerRadius: 18).stroke(border)) }
}
